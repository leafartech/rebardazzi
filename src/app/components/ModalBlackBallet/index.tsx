...
export default function ModalBlackBallet({ modal, setModal }: ModalProps) {
    var data: any = {};
    ...
    async function formSubmited() {
        await fetch(`https://53be2cd2bb8d905869263e2c944c03ec.api.mailchimp.com/3.0/lists/d6c9c23fba/members`, {
            method: 'POST',
            headers: {
            ...
            .then(response => {
                if (response.status === 200) {
                    router.push('https://rebardazzi.com.br/obrigado-black-november-ballet')
                } else {
                    alert('Houve um erro na inscrição. Por favor, tente novamente.');
                }
            }).catch(() =>  router.push('https://rebardazzi.com.br/obrigado-black-november-ballet'))
    }
    ...
    return (
        <div className={`fixed left-0 top-0 w-full h-screen flex justify-center items-center z-50 ${modal ? 'block' : 'hidden'}`}>
            ...
            <div id="content"></div>
            <form className="w-[440px] py-12 px-12 z-50 rounded-sm bg-[#6c2523] validate" action="https://gmail.us13.list-manage.com/subscribe/post?u=f8afbe3d4af930883e1a7e8cd&amp;id=d6c9c23fba&amp;f_id=001609e3f0" method="post" id="mc-embedded-subscribe-form" name="mc-embedded-subscribe-form" onSubmit={() => formSubmited()}>
                ...
                <div className="">
                    ...
                    <input type="hidden" name="MMERGE3" className=" text" id="mce-MMERGE3" value={'black-ballet-2023'} />
                </div>
                <button type="submit" className="relative w-full tracking-widest text-base sub flex justify-center items-center py-2 bg-green-600 mt-4 text-white rounded-full">
